...
export default function SmartLifeOS() {
  ...
  const anniversaryEvents = anniversaries.flatMap((a: any) =>
  ...
  const displayEvents: any[] = [...events, ...anniversaryEvents, ...routineEvents, ...subEvents, ...timetableEvents]
    .flatMap((e: any) => {
  ...
  const fetchEvents = async () => {
    ...
    // 🌟 修正：ループやフリーズを防ぐため、まずは確実にローカルデータを最優先で読み込む
    const localData = localStorage.getItem("events");
    if (localData) {
      rawData = JSON.parse(localData);
    }

    // クラウド同期はバックグラウンドで静かに行う（画面をフリーズさせない）
    if (isPremiumUser) {
      try {
  ...
  const renderEventContent = (arg: any) => {
        ...
        return (
          <div
            ...
            style={{
              ...
              boxShadow: `0 2px 6px ${hexToRgba(cColor, 0.15)}`,
              backdropFilter: "blur(6px)",
              WebkitBackdropFilter: "blur(6px)", // iOS Safari用
            }}
          >
    ...
    if (viewType === "timeGridDay") {
      return (
        <div
          className={`${highlightClass} smart-event-container force-full-width`}
          style={{
            height: "100%",
            width: "100%",
            overflow: "hidden",
        ...
        <header style={{ padding: "8px 12px 4px 12px", position: "relative", background: "linear-gradient(180deg, var(--bg-main) 40%, transparent 100%)", display: "flex", justifyContent: "space-between", alignItems: "center", zIndex: 100, pointerEvents: "auto" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px", zIndex: 20 }}>
            <button
              ...
              style={{ width: "44px", height: "44px", fontSize: "1.4rem", background: "var(--card-bg)", border: `1px solid var(--theme)`, borderRadius: "14px", color: "var(--theme)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: `0 0 10px var(--theme-shadow)`, transition: "all 0.2s", paddingBottom: "2px", flexShrink: 0 }}
            >
              ☰
            </button>
          ...
          <div style={{ position: "absolute", left: "50%", transform: "translateX(-50%)", zIndex: 10, display: "flex", justifyContent: "center" }}>
            <div style={{ position: "relative", display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "center", padding: "6px 16px", gap: "12px", background: "var(--card-bg)", border: `1px solid var(--theme)`, borderRadius: "32px", cursor: "pointer", boxShadow: `0 0 15px var(--theme-shadow), inset 0 0 8px var(--theme-shadow)` }}>
              ...
              <button
                ...
                style={{ border: "none", background: "transparent", color: "var(--theme)", fontWeight: "900", fontSize: "1.2rem", cursor: "pointer", padding: 0, position: "relative", zIndex: 20 }}
              >
                ◀
              </button>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px", zIndex: 20, height: "44px" }}>
            ...
            <div
              style={{
                ...
                display: "flex",
                alignItems: "center",
                boxShadow: `0 0 10px var(--theme-shadow)`,
                overflow: "hidden",
              ...
              {isViewSelectorExpanded ? (
                <div style={{ display: "flex", width: "100%", height: "100%", animation: "fadeIn 0.3s" }}>
                  <button
                    ...
                    style={{ flex: 1, height: "100%", padding: 0, background: viewType === "dayGridMonth" ? "var(--theme)" : "transparent", color: viewType === "dayGridMonth" ? "#fff" : "var(--theme)", border: "none", fontWeight: "900", fontSize: "0.8rem", cursor: "pointer", transition: "background 0.2s" }}
                  >
                    月
                  </button>
                  <button
                    ...
                    style={{ flex: 1, height: "100%", padding: 0, background: viewType === "timeGridWeek" ? "var(--theme)" : "transparent", color: viewType === "timeGridWeek" ? "#fff" : "var(--theme)", border: "none", fontWeight: "900", fontSize: "0.8rem", cursor: "pointer", transition: "background 0.2s", borderLeft: "1px dashed var(--theme-shadow)", borderRight: "1px dashed var(--theme-shadow)" }}
                  >
                    週
                  </button>
                  <button
                    onClick={() => {
                      setViewType("timeGridDay");
                      calendarRef.current?.getApi().changeView("timeGridDay");
                      calendarRef.current?.getApi().today(); // 🌟 今日へ移動
                    ...
                    style={{ flex: 1, height: "100%", padding: 0, background: viewType === "timeGridDay" ? "var(--theme)" : "transparent", color: viewType === "timeGridDay" ? "#fff" : "var(--theme)", border: "none", fontWeight: "900", fontSize: "0.8rem", cursor: "pointer", transition: "background 0.2s" }}
                  >
                    日
                  </button>
                </div>
              ) : (
                <button onClick={() => setIsViewSelectorExpanded(true)} style={{ width: "100%", height: "100%", padding: 0, background: "transparent", color: "var(--theme)", border: "none", fontWeight: "900", fontSize: "0.95rem", cursor: "pointer" }}>
          ...
          <div className="glass-panel" style={{ position: "absolute", top: "2px", left: "0px", right: "0px", bottom: "16px", padding: "2px 4px", borderRadius: "20px", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {viewType === "timeGridDay" &&
              (() => {
                ...
                // 🌟 修正：すべての終日予定の色を取得し、中央の円を薄く塗りつぶす
                    ...
                    <div style={{ flexShrink: 0, display: "flex", justifyContent: "center", marginBottom: "20px", padding: "10px 0" }}>
                      <div style={{ position: "relative", width: "280px", height: "280px" }}>
                        <svg viewBox="0 0 160 160" style={{ width: "100%", height: "100%", overflow: "visible", filter: "drop-shadow(0 8px 16px rgba(0,0,0,0.06))" }}>
                          ...
                          {/* 👇 修正：枠線(stroke)は変えずに、背景色(fill)だけをもっと薄くする */}
                          ...
                          <g transform="rotate(-90 80 80)">
                            {dayEvents.map((e: any, idx: number) => {
                              ...
                              const isTransit = e.extendedProps?.isTransitEvent || false; // 🌟 交通機関判定
                              ...
                              elements.push(
                                <circle
                                  ...
                                  fill="none"
                                  stroke={cColor}
                                  strokeWidth={isTransit ? "8" : "24"} // 🌟 交通機関の線は細く
                                  strokeDasharray={isTransit ? `4 6` : `${((eMin - sMin) / 1440) * CMain} ${CMain}`} // 🌟 交通機関は点線
                                  strokeDashoffset={isTransit ? 0 : -(sMin / 1440) * CMain}
                                  opacity={isTransit ? "0.6" : "0.95"} // 🌟 交通機関は少し薄く
                                />,
                              );
            ...
            <FullCalendar
              ...
              select={(info: any) => {
                ...
                // 🌟 修正: 月カレンダーでの複数日選択、または終日エリアの選択の場合
                if (info.allDay || viewType === "dayGridMonth") {
                  // 終了日が翌日の0時になっているので1日マイナスする
                  ...
                  setIsAllDayBackground(true); // 自動で終日をオンにする
                } else {
                  // 週カレンダーや日カレンダーで「時間」を選択した場合
                  setStartDate(toLocalYYYYMMDD(s));
                  setEndDate(toLocalYYYYMMDD(e));

                  setStartH(String(s.getHours()).padStart(2, "0"));
                  setStartM("00"); // 分は00に丸める
                  setEndH(String(e.getHours()).padStart(2, "0"));
                  setEndM("00");
              ...
              eventClick={(info: any) => {
                // 🌟 タップして詳細を開く際、裏で選択状態が残って次に進めなくなるバグ（ゴースト選択）を防ぐ
                setTimeout(() => {
              ...
              dateClick={(info: any) => {
                ...
                // 🌟 【完璧なバリア】指を離した時ではなく「指を置いた瞬間」の状態で判定する！
                if (!clipboardEvent && wasEventSelectedRef.current) {
                  calendarRef.current?.getApi().unselect();
                  return;
                }
              ...
              eventResizeStop={(info: any) => {
                setTimeout(() => {
                  ...
                  blockCalendarClick.current = false;
                  isSwipingRef.current = false;
                }, 500); // 🌟 リサイズ後も0.5秒間は背景タップを無視
                // 🌟 追加：リサイズ終了時に選択状態を強制解除
                info.view.calendar.unselect();
              }}
              selectLongPressDelay={200} // 🌟 0.5秒待たずに、触れて0.2秒ですぐ範囲選択が始まるように修正
                ...
                if (arg.view.type === "dayGridMonth") {
                  return (
                    <div style={{ position: "relative", display: "flex", justifyContent: "center", width: "100%", height: "100%" }}>
                      {/* 🌟 修正：標準クラス名を復活させ、日曜・祝日を赤く染めるCSSを連動させる */}
          ...
          if (graphSpan === "month") {
            graphData = Array.from({ length: 12 }, (_, i) => {
              ...
              targetEvts.forEach((e: any) => {
                ...
                // 🌟 時給計算（calculatedWage）が設定されていれば、それも収入に合算する
          ...
          <div className="modal-content glass-panel" onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: "400px", borderRadius: "28px", border: "1px solid var(--glass-border)", padding: "24px", background: "var(--bg-main)", color: "var(--text-main)", display: "flex", flexDirection: "column" }}>
            ...
            {cropImageSrc ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                ...
                <button onClick={() => setIsProfileModalOpen(false)} className="btn-pop" style={{ width: "100%", marginTop: "16px", padding: "14px" }}>
                  保存して閉じる
                </button>
              </div>
            )}
            ...
            {mode === "expense" ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "16px", padding: "10px 0" }}>
                ...
                <div style={{ display: "flex", gap: "12px", marginTop: "12px" }}>
                  <button onClick={() => setIsModalOpen(false)} className="btn-secondary" style={{ flex: 1 }}>
                    キャンセル
                  </button>
                  <button
                    onClick={async () => {
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "20px", padding: "10px 0" }}>
                ...
                <button onClick={() => setIsModalOpen(false)} className="btn-secondary" style={{ width: "100%" }}>
                  閉じる
                </button>
              </div>
            ) : (
              <>
                            ...
                            {methods.map((m) => {
                              ...
                              return (
                                <div
                                  ...
                                  style={{ padding: "8px", display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "0.75rem", fontWeight: "bold", color: "var(--text-main)", background: value === m.id ? "var(--input-bg)" : "transparent", borderRadius: "8px" }}
                                >
                                  <OptIcon size={14} color={m.color} style={{ flexShrink: 0 }} />
                                  {m.label}
                                </div>
                      ...
                      .flatMap((e: any) => {
                        ...
                        if (metadata.customFields?.isTransit) {
                          ...
                          if (metadata.customFields.hasReturnTransit && metadata.customFields.returnTransitDepTime && metadata.customFields.returnTransitArrTime) {
                            ...
                            const retEnd = new Date(retBase);
                            retEnd.setHours(ah, am, 0);
                            if (retEnd.getTime() < retStart.getTime()) {
                    ...
                    return (
                      <div ref={containerRef} style={{ position: "relative", width: "100%", height: "36px" }}>
                        <div className="pop-input" style={{ width: "100%", height: "100%", padding: "0 8px", display: "flex", alignItems: "center", cursor: "text", position: "relative", gap: "4px" }}>
                          <input type="text" style={{ flex: 1, border: "none", background: "transparent", outline: "none", fontSize: "0.8rem", color: "var(--text-main)", width: "100%", minWidth: 0 }} placeholder="相手の名前" value={value} onChange={(e) => onChange(e.target.value)} onFocus={() => setIsOpen(true)} />
                          {pastPayees.length > 0 && (
                            <div
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              style={{ cursor: "pointer", color: "var(--text-sub)", display: "flex", alignItems: "center", padding: "4px" }}
                            >
                              <ChevronDown size={14} />
                            </div>
                          )}
                  ...
                  const BlockLocationAndGathering = (
                    <div key="loc_gathering" style={{ marginBottom: "16px", display: "flex", flexDirection: "column", gap: "12px" }}>
                      ...
                      <div className="card-box" style={{ margin: 0, padding: 0, background: customFieldsData.isExpenseSet ? "var(--input-bg)" : "transparent", borderStyle: customFieldsData.isExpenseSet ? "solid" : "dashed", overflow: "visible" }}>
                        {" "}
                        {/* 👈 hidden を visible に変更しリスト見切れを解消 */}
                ...
                {customFieldsData.isTimetableEvent ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginTop: "24px" }}>
                    {/* 上段：キャンセル と 保存 */}
                    <div style={{ display: "flex", gap: "8px" }}>
                      <button onClick={() => setIsModalOpen(false)} className="btn-secondary" style={{ flex: 1, padding: "14px", fontSize: "0.95rem", fontWeight: "bold" }}>
                        キャンセル
                      </button>
