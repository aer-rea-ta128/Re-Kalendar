...
export default function Sidebar({ isSidebarOpen, setIsSidebarOpen, setOpenSections, themeColor, searchQuery, setSearchQuery, handleSearchExecute, setIsSearchMode, setIsColorPickerOpen, isDarkMode, setIsDarkMode, events, categories, targetType, setTargetType, targetValue, setTargetValue, currentMonthEvents, currentYearEvents, quickTemplates, setQuickTemplates, setMode, setStartDate, setEndDate, setStartH, setStartM, setEndH, setEndM, setTitle, setLocation, setMemo, setPhotoUrls, setIsStocked, setIsTentative, setRating, setIsPinned, setIsModalOpen, setCategoryName, setIsAllDayBackground, setEventColor, setIsAnalyticsModalOpen, setIsGalleryOpen, setIsCategoryModalOpen, setIsRoutineModalOpen, setIsAnniversaryModalOpen, syncWithCloud, handleEventClick, setCustomFieldsData, homeLocation, setHomeLocation, nearestStation, setNearestStation, walkTime, setWalkTime, startPointType, setStartPointType, displayMode, setDisplayMode, viewType, calendarCategoryFilter, setCalendarCategoryFilter, activeUserId, activeUserName, activeUserAvatar, setActiveUserAvatar, setActiveUserName, onLogout, setIsFinanceGraphOpen, setIsScheduleAssistantOpen, setIsAdvanceModalOpen, setIsTimetableModalOpen, setIsTemplateModalOpen, userProfile, setIsProfileModalOpen, isNotificationEnabled, setIsNotificationEnabled }: SidebarProps) {
  ...
  const [favoriteItems, setFavoriteItems] = useState<string[]>(() => {
    ...
    try {
      return saved ? JSON.parse(saved) : ["create_event", "finance_single"];
    } catch (e) {
      return ["create_event", "finance_single"];
    }
  });
  ...
  // 🌟 修正: システム自動連動の useEffect を削除し、完全に手動で切り替えられるようにしました。
  ...
  const ModalHeader = ({ title, onClose }: any) => (
    <div className="modal-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
      <h2 className="modal-title" style={{ margin: 0, color: themeColor, fontSize: "1.4rem", fontWeight: 900 }}>
        {title}
      </h2>
      <button onClick={onClose} className="btn-close" style={{ background: "var(--input-bg)", border: "1px solid var(--border-color)", borderRadius: "50%", width: "36px", height: "36px", fontSize: "1.2rem", cursor: "pointer", color: "var(--text-sub)", fontWeight: "bold", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ×
      </button>
    </div>
  );
  ...
  return (
    <>
      <div
        ...
        style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100dvh", background: "rgba(0,0,0,0.6)", backdropFilter: "blur(2px)", zIndex: 1999, opacity: isSidebarOpen ? 1 : 0, pointerEvents: isSidebarOpen ? "auto" : "none", transition: "all 0.3s ease" }}
      />

      {/* 🌟 修正: アプリ全体のレイアウトを壊さないよう、固定値(64px)でサイドバーだけを安全に押し下げる */}
      <div style={{ position: "fixed", top: 0, left: 0, width: "85%", maxWidth: "320px", height: "100dvh", borderTopRightRadius: "24px", borderBottomRightRadius: "24px", zIndex: 2000, transform: isSidebarOpen ? "translateX(0)" : "translateX(-100%)", paddingTop: "64px", paddingRight: "20px", paddingBottom: "24px", paddingLeft: "20px", transition: "transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1)", display: "flex", flexDirection: "column", background: "var(--bg-main)", boxShadow: isSidebarOpen ? "4px 0 24px rgba(0,0,0,0.15)" : "none", borderRight: "1px solid var(--border-color)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "20px", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <button
              ...
              style={{ flex: 1, padding: "8px", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", fontSize: "0.8rem", borderRadius: "12px" }}
            >
              <div style={{ width: "14px", height: "14px", borderRadius: "50%", backgroundColor: themeColor }} /> テーマ
            </button>

            {/* 🌟 修正: 手動で切り替え可能なトグルボタンとして復活 */}
            <button onClick={() => setIsDarkMode(!isDarkMode)} className="btn-secondary" style={{ flex: 1, padding: "8px", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", fontSize: "0.75rem", borderRadius: "12px", background: isDarkMode ? "#1e293b" : "#f8fafc", color: isDarkMode ? "#f8fafc" : "#1e293b", border: `1px solid ${isDarkMode ? "#334155" : "#e2e8f0"}`, fontWeight: "bold", cursor: "pointer", transition: "all 0.2s" }}>
              {isDarkMode ? (
                <>
                  <Moon size={14} /> ダーク
                </>
        ...
        <div style={{ flex: 1, overflowY: "auto", paddingRight: "5px", display: "flex", flexDirection: "column", gap: "24px" }} className="hide-scrollbar">
          ...
          <div>
            <div onClick={() => setIsKeepSectionOpen(!isKeepSectionOpen)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", marginBottom: "8px", padding: "0 4px" }}>
              ...
              <div style={{ background: "var(--input-bg)", padding: "10px", borderRadius: "16px", border: "1px solid var(--border-color)", display: "flex", flexDirection: "column", gap: "8px" }}>
                {(() => {
                  ...
                  return keepEvents.map((e: any) => {
                    ...
                    return (
                      <div key={e.id} style={{ background: "var(--card-bg)", padding: "12px", borderRadius: "12px", borderLeft: `4px solid ${cColor}`, border: "1px solid var(--border-color)", display: "flex", flexDirection: "column", gap: "6px", boxShadow: "0 2px 4px rgba(0,0,0,0.01)" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "6px" }}>
                          <div style={{ fontSize: "0.85rem", fontWeight: "bold", color: "var(--text-main)", wordBreak: "break-all" }}>{e.title}</div>
                          <span style={{ fontSize: "0.6rem", fontWeight: "bold", color: "#f59e0b", background: "rgba(245,158,11,0.1)", padding: "4px 8px", borderRadius: "6px", flexShrink: 0 }}>仮予定</span>
                        </div>

                        {e.start && <div style={{ fontSize: "0.7rem", color: "var(--text-sub)", fontWeight: "bold" }}>予定日: {e.start.split("T")[0].replace(/-/g, "/")}</div>}
        ...
        <div style={{ marginTop: "auto", paddingTop: "16px", borderTop: "1px solid var(--border-color)", display: "flex", flexDirection: "column", gap: "8px", flexShrink: 0 }}>
          {activeUserId && (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: "10px", overflow: "hidden" }}>
                <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "var(--input-bg)", border: "2px solid var(--theme)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--theme)", flexShrink: 0, overflow: "hidden" }}>{userProfile?.avatar ? <img src={userProfile.avatar} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: `${userProfile.avatarPanX || 50}% ${userProfile.avatarPanY || 50}%`, transform: `scale(${userProfile.avatarScale || 1})` }} alt="profile" /> : <User size={20} />}</div>
          ...
          <button
            ...
            style={{ width: "100%", padding: "12px", borderRadius: "12px", background: "var(--input-bg)", border: "none", color: "var(--text-main)", fontSize: "0.85rem", cursor: "pointer", display: "flex", alignItems: "center", gap: "10px", fontWeight: "bold", transition: "background 0.2s" }}
          >
        ...
        <div className="modal-overlay" onClick={() => setIsSettingsPanelOpen(false)} style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)", zIndex: 9999, display: "flex", justifyContent: "center", alignItems: "center", padding: "15px" }}>
          <div className="modal-content glass-panel" onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: "440px", borderRadius: "28px", border: "1px solid var(--glass-border)", padding: "24px", background: "var(--bg-main)", color: "var(--text-main)", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            ...
            <div className="hide-scrollbar" style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: "24px", paddingRight: "4px" }}>
              {/* 🌟 10分前通知の設定をここにお引越し */}
              <div>
                <span style={{ fontSize: "0.8rem", fontWeight: "bold", color: "var(--theme)", marginBottom: "12px", display: "block" }}>通知の設定</span>
                <div style={{ background: "var(--card-bg)", padding: "16px", borderRadius: "16px", border: "1px solid var(--border-color)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "0.85rem", fontWeight: "bold", color: "var(--text-main)" }}>
                    <Bell size={18} color="var(--theme)" /> 予定の10分前通知
                  </span>
